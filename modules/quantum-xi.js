...
// Opaque debug logger (toggle with globalThis.__SUIQ_DEBUG = false)
const QLOG = {
  enabled: globalThis.__SUIQ_DEBUG ?? true,
  i(code, data) { if (!this.enabled) return; console.log(`§Q:${code}`, data ?? ""); },
  w(code, data) { if (!this.enabled) return; console.warn(`§Q:${code}`, data ?? ""); },
  e(code, data) { console.error(`§Q:${code}`, data ?? ""); }
};
...
export async function onQuantumImport(systemId, text) {
  ...
  ui.notifications.info(
    game.i18n.localize("SUIQ.UI.Import.Success")
  );
...
export async function onQuantumImportJournalPage(systemId, text) {
  ...
  try {
    ...
    QLOG.i("IJ0", { g: game.user.isGM ? 1 : 0, s: state, n: raw.length });
    ...
    const match = dict[dKey];
    QLOG.i("IJ3", { l: lang === "fr" ? 1 : 0, m: match ? 1 : 0 });

    if (!match) return false;

    const encoded = match[state];
    QLOG.i("IJ4", { s: state, e: encoded ? 1 : 0 });
    ...
    QLOG.i("IJ7", { c: chain ? 1 : 0, k: Array.isArray(chain) ? chain.length : 0 });
    ...
    // NOTE: keep original behavior (second import call)
    await importSequentialPages([`${journalUUID}::${pageIndex}`]);

    QLOG.i("IJ9", (performance.now() - t0) | 0);
    return true;
  } catch (e) {
    QLOG.e("IJX", e);
    return false;
...
export async function onQuantumShowPage(systemId, text) {
  ...
  try {
    ...
    QLOG.i("SP0", { g: game.user.isGM ? 1 : 0, s: state, n: raw.length });
    ...
    const match = dict[dKey];
    QLOG.i("SP3", { l: lang === "fr" ? 1 : 0, m: match ? 1 : 0 });

    if (!match) return false;

    const encoded = match[state];
    QLOG.i("SP4", { s: state, e: encoded ? 1 : 0 });
    ...
    if (Array.isArray(chain) && chain.length) {
      const fullChain = [decoded, ...chain].filter((v, i, a) => a.indexOf(v) === i);
      QLOG.i("SP7", fullChain.length);
      await revealSequentialDisplay(fullChain);
      QLOG.i("SP9", (performance.now() - t0) | 0);
      return true;
    }
